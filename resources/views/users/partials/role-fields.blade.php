...
<div x-data="{ role: @js($roleValue) }" class="space-y-4">
    <div>
        ...
        <select id="role" name="role" x-model="role" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-900 dark:text-gray-300 shadow-sm focus:border-sky-500 focus:ring-sky-500 text-sm" required>
            ...
            <div class="max-h-48 overflow-y-auto rounded-md border border-gray-200 dark:border-gray-600 p-3 space-y-2">
                @foreach ($assignableCities as $city)
                    <label class="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
                        <input type="checkbox" name="city_ids[]" value="{{ $city->id }}" class="rounded border-gray-300 dark:border-gray-600 text-sky-600"
    ...
    @if ($showActiveToggle && $actor->isAdmin())
        <div class="flex items-center gap-2">
            <input id="is_active" type="checkbox" name="is_active" value="1" class="rounded dark:bg-gray-900 border-gray-300 dark:border-gray-700 text-sky-600 shadow-sm focus:ring-sky-500"
