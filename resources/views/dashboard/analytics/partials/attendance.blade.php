...
<div class="space-y-4">
    <p class="text-sm text-gray-600 dark:text-gray-300 leading-relaxed">
        {{ __('Área reservada a frequência escolar, faltas e alertas. O gráfico abaixo é ilustrativo até mapear o diário de classe; exporte em PNG quando precisar anexar ao relatório.') }}
    </p>
    @if (! empty($attendanceData['message']))
        <p class="text-sm text-gray-600 dark:text-gray-300">{{ $attendanceData['message'] }}</p>
    @endif

    @if (! empty($attendanceData['chart']))
        <x-dashboard.chart-panel :chart="$attendanceData['chart']" exportFilename="frequencia" />
    @else
