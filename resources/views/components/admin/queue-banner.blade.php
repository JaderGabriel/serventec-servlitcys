...
<div {{ $attributes->merge(['class' => 'rounded-lg border border-sky-200 bg-sky-50 dark:border-sky-800 dark:bg-slate-800 '.($compact ? 'px-3 py-2.5' : 'px-4 py-3')]) }}>
    <div class="flex flex-wrap items-start justify-between gap-3">
        <div class="min-w-0">
            <p class="{{ $compact ? 'text-xs' : 'text-sm' }} font-semibold text-sky-950 dark:text-sky-100">
                {{ __('Processamento em fila') }}
            </p>
            <p class="{{ $compact ? 'text-[11px] mt-0.5' : 'text-sm mt-1' }} text-sky-900 dark:text-sky-100 leading-relaxed">
            ...
            @if (! $compact)
                <p class="mt-1.5 text-xs text-sky-800 dark:text-sky-200">
