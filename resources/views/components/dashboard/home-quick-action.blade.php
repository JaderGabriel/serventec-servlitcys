...
@php
    ...
    $badgeToneClass = match ($badgeTone) {
        'ok' => 'bg-teal-100 text-teal-800 dark:bg-teal-950/60 dark:text-teal-200',
...
>
    <span class="serv-qa-card__accent pointer-events-none absolute inset-y-3 start-0 w-1 rounded-e-full opacity-0 transition-opacity group-hover:opacity-100 @if ($accent === 'teal') bg-teal-500 @elseif ($accent === 'indigo') bg-indigo-500 @elseif ($accent === 'amber') bg-amber-500 @else bg-slate-400 @endif" aria-hidden="true"></span>
    @if (filled($badge))
    ...
    <span class="serv-qa-card__body min-w-0 flex-1 pe-6 space-y-0.5">
        ...
        @endif
        <span class="serv-qa-card__title block font-display text-sm font-semibold text-slate-900 dark:text-slate-100 leading-snug group-hover:text-teal-800 dark:group-hover:text-teal-300 {{ $featured ? 'text-base' : '' }}">{{ $title }}</span>
    ...
    <span class="serv-qa-card__go absolute end-3 top-1/2 -translate-y-1/2 text-slate-300 dark:text-slate-600 group-hover:text-teal-600 dark:group-hover:text-teal-400" aria-hidden="true">
