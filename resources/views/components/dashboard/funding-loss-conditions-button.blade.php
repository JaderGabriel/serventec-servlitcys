...
@php
    ...
    $programIds = is_array($activeProgramIds) ? array_values(array_filter($activeProgramIds, static fn ($id): bool => is_string($id) && $id !== '')) : [];
@endphp
...
<button
    ...
    title="{{ __('FUNDEB, VAAR, PNAE, PNATE, PDDE — condições de perda') }}"
    aria-label="{{ __('Financiamentos públicos — condições de perda') }}"
    x-on:click="$dispatch('funding-loss-set-active', { ids: @js($ids), programIds: @js($programIds) }); $dispatch('open-modal', 'funding-loss-conditions')"
>
