                ...
                <section class="serv-panel overflow-hidden min-w-0 serv-horizonte-gis" aria-labelledby="horizonte-map-heading">
                            ...
                            <div class="flex flex-wrap items-center gap-2 text-xs">
                                <label class="inline-flex items-center gap-2">
                                    ...
                                    <select
                                        ...
                                        :disabled="pageLoading || regionalLoading"
                                        class="rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-900 dark:text-gray-200 text-sm min-w-[12rem]"
                                    >
                    ...
                    <div class="px-5 py-4 border-b border-slate-200/90 dark:border-slate-700/90 space-y-4">
                        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                            ...
                            <p class="text-xs text-slate-500 tabular-nums" x-show="!pageLoading">
                                ...
                                <span x-show="isRegionalMode" x-cloak>
                                    <span x-text="markers.length.toLocaleString('pt-BR')"></span> {{ __('em') }}
                                    <span x-text="ufLabel(scopeUf)"></span>
                                    <span x-show="filteredCount !== markers.length" x-cloak>
                                        · <span x-text="filteredCount.toLocaleString('pt-BR')"></span> {{ __('com filtro') }}
                                    </span>
                        ...
                        >
                            ...
                            · <span x-text="markers.length.toLocaleString('pt-BR')"></span> {{ __('com dados na UF') }}
                            <span x-show="filteredCount !== markers.length" x-cloak>
                                    ...
                                    <template x-if="active">
                                        <div class="space-y-3">
                                            ...
                                            <div x-show="canManageSge && active && canEditSgeFor(active)" x-cloak class="pt-2 border-t border-slate-200/80 dark:border-slate-700/80 space-y-2">
                                                ...
                                                <div class="flex flex-wrap gap-2">
                                                    <button
                                                        type="button"
                                                        class="serv-btn-secondary text-xs"
                                                        @click.stop="openSgeForm(active, { readOnly: true })"
                                                    >
                                                        {{ __('Consultar') }}
                                                    </button>
                                                    <button
                                            ...
                                            <div x-show="canManageSge && active && active.in_catalog" x-cloak class="pt-2 border-t border-slate-200/80 dark:border-slate-700/80 space-y-2">
                                                ...
                                                <div class="flex flex-wrap gap-2">
                                                    <a
                                                        x-show="active.cities_url"
                                                        :href="active.cities_url"
                                                        target="_blank"
                                                        rel="noopener noreferrer"
                                                        class="serv-btn-secondary text-xs"
                                                    >{{ __('Consultar ficha') }}</a>
                                                    <a
                                                        x-show="active.analytics_url"
                                                        :href="active.analytics_url"
                                                        target="_blank"
                                                        rel="noopener noreferrer"
                                                        class="serv-btn-primary text-xs"
                                                    >{{ __('Abrir consultoria') }}</a>
                                                </div>
                                            </div>
                                        </div>
                                    </template>
                                ...
                                >
                                    <div class="serv-horizonte-sge-panel" @click.stop>
                                        ...
                                        <form class="space-y-3" @submit.prevent="saveSgeEntry()">
                                            ...
                                            <p x-show="sgeFormReadOnly && !sgeForm.has_entry && !sgeForm.system" class="text-xs text-slate-500 dark:text-slate-400">
                                                {{ __('Sem registo Horizonte para este município — use «Cadastrar» para criar.') }}
                                            </p>
                                            <p x-show="sgeFormError" x-text="sgeFormError" class="text-xs text-red-600 dark:text-red-400"></p>
                                            <div class="flex flex-wrap items-center justify-between gap-2 pt-1">
                                                <button
                                                    type="button"
                                                    x-show="!sgeFormReadOnly && sgeForm.has_entry"
                                                ...
                                                <div class="flex gap-2 ms-auto">
                                                    ...
                                                    <button
                                                        type="button"
                                                        x-show="sgeFormReadOnly"
                                                        class="serv-btn-primary text-xs"
                                                        @click="enableSgeFormEdit()"
                                                    >{{ __('Editar') }}</button>
