@if (session('admin_sync_queued'))
    ...
    <div
        ...
        x-show="show"
        x-transition
        class="rounded-lg border border-emerald-200 bg-emerald-50 dark:border-emerald-800 dark:bg-slate-800 px-4 py-3 text-sm"
        role="status"
    >
        <div class="flex flex-wrap items-start justify-between gap-3">
            <div class="min-w-0">
                <p class="font-semibold text-emerald-950 dark:text-emerald-100">{{ __('Pedido enviado para a fila') }}</p>
                <p class="mt-1 text-emerald-900 dark:text-emerald-100">
                    {{ __('A tarefa será processada em segundo plano. Atualize a fila para ver quando concluir.') }}
                </p>
                @if ($taskId > 0)
                    <p class="mt-1 text-xs text-emerald-800 dark:text-emerald-200 font-mono">#{{ $taskId }}</p>
                @endif
            </div>
            <div class="flex flex-wrap gap-2 shrink-0">
                ...
                @endif
                <a href="{{ route(($syncQueueRoutePrefix ?? 'admin.sync-queue').'.index') }}" class="inline-flex items-center rounded-lg border border-emerald-400 dark:border-emerald-600 bg-white dark:bg-emerald-900 px-3 py-1.5 text-xs font-medium text-emerald-900 dark:text-emerald-100 hover:bg-emerald-100 dark:hover:bg-emerald-800">
                    {{ __('Fila') }}
                </a>
