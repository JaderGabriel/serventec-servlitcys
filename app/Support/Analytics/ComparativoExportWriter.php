...
final class ComparativoExportWriter
{
    /**
    ...
    public static function streamCsv(array $rows): void
    {
        ...
        fputcsv($out, ComparativoExportRowsBuilder::columnLabels(), ';');
        foreach ($rows as $row) {
    ...
    public static function writeXlsx(string $absolutePath, array $rows): void
    {
        ...
        $active->setTitle(__('Comparativo'));

        $headers = ComparativoExportRowsBuilder::columnLabels();
    ...
    {
        $line = [];
        foreach (ComparativoExportRowsBuilder::columnHeaders() as $key) {
            $line[] = (string) ($row[$key] ?? '');
        }
