    ...
    private function feedPhases(array $fundebSet, array $censoSet, array $saebSet, ?string $microdadosPath, int $ibgeUfsWarmed, int $ibgeUfsTotal, int $cadunicoCount, int $demographyCount, array $educacensoWindow, int $educacensoYearsIndexed): array
    {
        $phases = [
            ...
            [
                ...
                'description' => __('Importa microdados INEP **ano a ano** para a janela do gráfico de matrículas (§6.9) — :n ano(s) por passo (HORIZONTE_EDUCACENSO_YEARS_PER_STEP). Repita até concluir todos os anos.', [
                    'n' => (string) max(1, (int) config('horizonte.fortnightly_feed.educacenso_years_per_step', 1)),
                ]),
                'source_id' => 'censo_inep_matriculas',
                'hub_anchor' => '#source-censo_inep_matriculas',
                ...
                'cli_reset' => 'php artisan horizonte:fortnightly-feed --phase=educacenso --reset',
                ...
                'metric_total' => count($educacensoWindow),
