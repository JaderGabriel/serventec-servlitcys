...
{
    public function test_monthly_documents_include_july_june_and_may(): void
    {
        $months = DocumentationEscalonadasCatalog::monthlyDocuments();
        $ids = array_column($months, 'id');

        $this->assertSame(['202607', '202606', '202605'], $ids);
        $this->assertSame('docs/ENTREGAS_ESCALONADAS_JULHO_2026.md', $months[0]['path']);
    }

    public function test_listed_paths_include_future_improvements(): void
    {
        $paths = DocumentationEscalonadasCatalog::listedPaths();

        $this->assertContains(DocumentationEscalonadasCatalog::indexPath(), $paths);
        $this->assertContains(DocumentationEscalonadasCatalog::futureImprovementsPath(), $paths);
        $this->assertContains('docs/ENTREGAS_ESCALONADAS_JULHO_2026.md', $paths);
    }
    ...
    public function test_menu_section_lists_index_future_and_monthly_submenu(): void
    {
        ...
        $this->assertSame(
            DocumentationEscalonadasCatalog::futureImprovementsPath(),
            $section['items'][1]['path'],
        );
        $this->assertSame(__('Por mês'), $section['submenus'][0]['title']);
        $this->assertCount(3, $section['submenus'][0]['items']);
    }
