...
| INF-05 | **P1** | Tuning InnoDB (buffer pool) + isolar telemetria Pulse (Redis) — raiz da contenção/lock wait | Em andamento — app documentada (2026-07-24); falta ops InnoDB em produção | [ESCALABILIDADE_INFRAESTRUTURA.md](ESCALABILIDADE_INFRAESTRUTURA.md) §3, §7 etapa 1 |
| INF-06 | P2 | App stateless (Redis sessão/cache/fila) + PDO persistente na base principal | Em andamento — `.env.example` + `DB_PERSISTENT` (2026-07-24) | [ESCALABILIDADE_INFRAESTRUTURA.md](ESCALABILIDADE_INFRAESTRUTURA.md) §6, §7 etapas 2–3 |
